...
export default function AddInternalContributorDialog({ isOpen, onClose }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedLecturer || !data?.id) return;
        
        setLoading(true);
        try {
            await addInternalContributorAction(data.id, { lecturerId: selectedLecturer.id, role });
            clear();
            onClose();
        } catch (error) {
            console.error("Lỗi khi gửi form thành viên nội bộ:", error);
        } finally {
            setLoading(false);
        }
    };
                                        ...
                                        <div className="absolute z-50 mt-1 max-h-56 w-full overflow-y-auto rounded-md border bg-popover text-popover-foreground shadow-md">
                                            {lecturerLoading ? (
                                                ...
                                                lecturerResults.map((l) => (
                                                    <button key={l.id} type="button" onClick={() => { setSelectedLecturer(l); setLecturerQuery(l.name); setDdOpen(false); }}
