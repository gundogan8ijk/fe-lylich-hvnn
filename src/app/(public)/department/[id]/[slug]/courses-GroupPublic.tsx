...
import { DisciplineCoursePublicList } from '@/working-public/department-Public/department-public-type';
import { getListCourseByPublicDisciplineAction } from '@/working-public/department-Public/department-public-hook';

export function CoursesGroupPublic({ departmentId, disciplineId }: { departmentId: string, disciplineId: string }) {
    const [searchQuery, setSearchQuery] = useState('');
    const [isSearch, setIsSearch] = useState(false);
    const [currentPage, setCurrentPage] = useState(1);
    const LocalPerPage = 12;
    const [courseList, setCourseList] = useState<DisciplineCoursePublicList | null>(null);
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const res = await getListCourseByPublicDisciplineAction(departmentId, disciplineId, queryParam);
    ...
    if (searchQuery.trim()) {
        ...
        filteredItems = filteredItems.filter(item =>
            item.courseName.toLowerCase().includes(lowerQuery) ||
                ...
                <div className="space-y-3 mb-8">
                    {paginatedItems.length > 0 ? (
                        paginatedItems.map((item) => (
                            <div
                                key={item.courseId}
                                className="p-5 bg-white border border-border rounded-xl transition-all duration-200"
                            >
