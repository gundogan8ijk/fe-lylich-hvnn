...
export default function AddContributorDialog({ open, onOpenChange, projectId }: Props) {
    ...
    useEffect(() => {
        if (open) {
            ...
            setLecturerQuery('')
            setLecturerResults([])
            setJoinedAt(new Date().toISOString().split('T')[0]) // Default hôm nay
        }
    }, [open])
    ...
    const handleSave = async () => {
        ...
        setSaving(true)
        try {
            const ok = await addContributorAction(projectId, {
            ...
            <DialogContent className="max-w-md bg-white text-slate-900 border-slate-200 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-800">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-slate-500 dark:text-slate-400">
                        Tìm kiếm giảng viên trong hệ thống và thêm vào ban nghiên cứu đề tài.
                    </DialogDescription>
                </DialogHeader>
