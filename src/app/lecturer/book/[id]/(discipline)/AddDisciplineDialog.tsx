...
import { addDisciplinesAction } from '@/working-Lecturer/Book-Detail/Book-Detail-hook';
...
export default function AddDisciplineDialog({ isOpen, onClose }: Props) {
  ...
  const existingDisciplineIds = book?.disciplines.map(d => d.id) ?? [];
  const maxDisciplines = book?.maxDisciplines ?? 5;
  const remainingSlots = maxDisciplines - (book?.disciplines.length ?? 0) - selectedList.length;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const success = await addDisciplinesAction(book.id, ids);
      if (success) {
        clearAll();
        onClose();
      }
              ...
              {ddOpen && remainingSlots > 0 && (
                <div className="absolute z-50 mt-1 max-h-56 w-full overflow-y-auto rounded-md border bg-popover shadow-md">
                  {disciplineLoading ? (
                    ...
                    <p className="px-3 py-2.5 text-xs text-muted-foreground">Không tìm thấy hoặc đã có trong sách</p>
                  ) : (
