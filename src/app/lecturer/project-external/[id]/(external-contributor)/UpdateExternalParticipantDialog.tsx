...
import { updateExternalParticipantAction } from '@/working-Lecturer/ProjectExternal-Detail/ProjectExternal-Detail-hook';
...
export default function UpdateExternalParticipantDialog({ participantId, onClose }: Props) {
    ...
    const [role, setRole] = useState<ProjectExternalMemberRoleName>(
        participant?.role ?? PROJECT_EXTERNAL_MEMBER_ROLE_OPTIONS[0]?.value ?? 'CoreTeam'
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!fullName.trim() || !project?.id) return;

        setLoading(true);
        try {
            const success = await updateExternalParticipantAction(project.id, participantId, {
            ...
            console.error('Lỗi cập nhật người tham gia:', error);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs animate-in fade-in duration-150">
            <div className="bg-background rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-border animate-in zoom-in-95 duration-150">
                <div className="sticky top-0 bg-background border-b border-border px-6 py-4 flex justify-between items-center z-10">
                    <h3 className="text-base font-semibold text-foreground">Cập nhật người tham gia bên ngoài</h3>
                    <button
                        type="button"
                        onClick={onClose}
                        disabled={loading}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div className="space-y-1.5">
                        <label className="block text-sm font-semibold text-foreground">Email (tuỳ chọn)</label>
                        <input
                            type="email"
                            disabled={loading}
