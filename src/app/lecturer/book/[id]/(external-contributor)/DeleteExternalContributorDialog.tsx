...
import { removeExternalContributorAction } from '@/working-Lecturer/Book-Detail/Book-Detail-hook';
...
export default function DeleteExternalContributorDialog({ externalContributorId, onClose }: Props) {
    ...
    const handleConfirm = async () => {
        ...
        setLoading(true);
        try {
            await removeExternalContributorAction(book.id, externalContributorId);
            onClose();
        } catch (error) {
            console.error('Lỗi xóa tác giả bên ngoài:', error);
        } finally {
            setLoading(false);
        }
    };
