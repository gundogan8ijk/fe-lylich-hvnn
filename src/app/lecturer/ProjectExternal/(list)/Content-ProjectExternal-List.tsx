...
export default function ContentProjectExternalList() {
    ...
    const { data, loading } = storeProjectExternalList();
    ...
    const handleViewDetail = (id: string) => {
        router.push(`/lecturer/project-external/${id}`);
    };
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Đề tài nghiên cứu ngoài</h1>
                    <p className="text-muted-foreground mt-2 text-sm sm:text-base">
                        Quản lý các đề tài, dự án hợp tác với tổ chức bên ngoài
                    </p>
                </div>
            ...
            {loading ? (
                ...
                <div className="grid gap-4">
                    {filtered.length === 0 ? (
                        ...
                        filtered.map((item) => (
                            <ProjectExternalCard key={item.id} item={item} onViewDetail={handleViewDetail} />
                        ))
                    )}
                </div>
            )}

            <RegisterProjectExternalDialog
                open={dialogOpen}
                onOpenChange={setDialogOpen}
                onSubmit={registerProjectExternalAction}
            />
        </div>
    );
}