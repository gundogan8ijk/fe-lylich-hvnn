...
import { Eye, Calendar, ExternalLink, Globe, Lock, User, Trophy } from 'lucide-react'
...
import { AwardManagerItemResponse } from '@/working-manager/award/award-manager-type'

type AwardManagerCardProps = {
    item: AwardManagerItemResponse
...
export default function AwardManagerCard({
    item,
    onViewDetail,
}: AwardManagerCardProps) {
    ...
    const gradient = statusGradient[item.confirmedStatus as ConfirmedStatus] ?? "from-orange-500 to-yellow-500"
                ...
                <div className="mb-3 flex flex-col gap-y-2">
                    <div className="flex items-center justify-between gap-2">
                        <div className="flex flex-wrap items-center gap-2">
                            <Trophy className="h-4 w-4 flex-shrink-0 text-orange-500" />
                    ...
                    <p className="flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 line-clamp-1 mt-0.5">
                        {getAwardLevelLabel(item.awardLevel)} • {item.awardDate ? getDateOnly(item.awardDate).slice(3) : 'Chưa cập nhật'}
                    </p>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
                    ...
                    <div className="flex items-center gap-x-2">
                        ...
                        <Button variant="ghost" size="sm"
                            ...
                            onClick={() => onViewDetail(item.awardId)}
                        >
