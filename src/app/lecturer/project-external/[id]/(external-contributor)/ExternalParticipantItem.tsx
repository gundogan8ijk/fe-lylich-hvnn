...
export default function ExternalParticipantItem({
    participant,
    disabled,
    roleLabel,
    onEditClick,
    onRemoveClick,
}: ExternalParticipantItemProps) {
    ...
    return (
        <div className="group flex justify-between items-center p-4 bg-background hover:bg-muted/30 rounded-xl border border-border shadow-2xs hover:shadow-sm transition-all duration-200">
            <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className="w-10 h-10 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-sm border border-indigo-100/60 shrink-0">
                    {getInitials(participant.fullName)}
                </div>
                <div className="space-y-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                        <p className="font-semibold text-foreground text-sm sm:text-base truncate">{participant.fullName}</p>
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 text-xs font-medium rounded-md border border-indigo-100">
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {participant.email && (
                            <>
                                ...
                                <span className="truncate max-w-[180px] sm:max-w-xs">{participant.email}</span>
                            </>
                        )}
                    </div>
                </div>
            ...
            {!disabled && (
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200">
                    <button
                        type="button"
                        onClick={() => onEditClick(participant.id)}
                    ...
                    <button
                        type="button"
                        onClick={() => onRemoveClick(participant.id)}
