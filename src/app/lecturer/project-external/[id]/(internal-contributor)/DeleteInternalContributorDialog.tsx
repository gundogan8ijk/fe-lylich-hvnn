...
import { removeInternalContributorAction } from '@/working-Lecturer/ProjectExternal-Detail/ProjectExternal-Detail-hook';
...
export default function DeleteInternalContributorDialog({ contributorId, onClose }: Props) {
    ...
    const handleConfirm = async () => {
        ...
        setLoading(true);
        try {
            await removeInternalContributorAction(project.id, contributorId);
            onClose();
        } catch (error) {
            console.error('Lỗi khi xóa thành viên nội bộ:', error);
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-background rounded-xl shadow-xl max-w-sm w-full border border-border animate-in zoom-in-95 duration-150">
                ...
                <div className="px-6 py-5">
                    <p className="text-sm text-muted-foreground leading-relaxed">
                        ...
                        khỏi danh sách tham gia đề tài này?
                    </p>
                </div>
