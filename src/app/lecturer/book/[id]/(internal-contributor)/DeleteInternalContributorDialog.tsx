...
export default function DeleteInternalContributorDialog({ contributorId, onClose }: Props) {
    const { data: book } = storeBookDetail();
    const [loading, setLoading] = useState(false);

    if (!contributorId) return null;

    const contributor = book?.internalContributors.find(c => c.id === contributorId);
    ...
    const handleConfirm = async () => {
        ...
        setLoading(true);
        try {
            await removeInternalContributorAction(book.id, contributorId);
            onClose();
        } catch (error) {
            console.error('Lỗi xóa tác giả nội bộ:', error);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs animate-in fade-in duration-150">
            <div className="bg-background rounded-xl shadow-xl max-w-sm w-full border border-border animate-in zoom-in-95 duration-150">
                <div className="px-6 py-4 border-b border-border flex items-center gap-2.5 text-destructive">
                    ...
                    <h3 className="text-base font-semibold text-foreground">Xác nhận gỡ tác giả</h3>
                </div>
                <div className="px-6 py-5">
                    <p className="text-sm text-muted-foreground leading-relaxed">
                        Bạn có chắc chắn muốn gỡ tác giả{' '}
                        <span className="font-semibold text-foreground">{contributor.fullName}</span>{' '}
                        khỏi danh sách đóng góp của sách này?
                    </p>
                </div>
