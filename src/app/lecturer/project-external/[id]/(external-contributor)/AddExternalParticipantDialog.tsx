...
export default function AddExternalParticipantDialog({ isOpen, onClose }: Props) {
    ...
    const clear = () => {
        setFullName('');
        setEmail('');
        setRole(PROJECT_EXTERNAL_MEMBER_ROLE_OPTIONS[0]?.value ?? 'CoreTeam');
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const success = await addExternalParticipantAction(project.id, {
            ...
            console.error('Lỗi thêm người tham gia bên ngoài:', error);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs animate-in fade-in duration-150">
            <div className="bg-background rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-border animate-in zoom-in-95 duration-150">
                <div className="sticky top-0 bg-background border-b border-border px-6 py-4 flex justify-between items-center z-10">
                    <h3 className="text-base font-semibold text-foreground">Thêm người tham gia bên ngoài</h3>
                    <button
                        type="button"
                        onClick={onClose}
                        disabled={loading}
