...
export default function AddDisciplineDialog({ isOpen, onClose }: Props) {
    ...
    const existingDisciplineIds = data?.disciplines.map(d => d.id) ?? [];
    const maxDisciplines = data?.maxDisciplines ?? 5; // lấy từ store
    const remainingSlots = maxDisciplines - (data?.disciplines.length ?? 0) - selectedList.length;
    ...
    const searchDisciplines = async (q: string) => {
        ...
        try {
            ...
            if (id === searchIdRef.current && res.code === 1 && res.data) {
                // Lọc bỏ các discipline đã có trong bài báo hoặc đã chọn trong dialog
                const filtered = res.data.filter(d => 
                    !existingDisciplineIds.includes(d.id) && 
                    !selectedList.some(s => s.id === d.id)
                );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const success = await addDisciplinesAction(data.id, ids);
            if (success) {
                clearAll();
                onClose();
            }
                            ...
                            {ddOpen && remainingSlots > 0 && (
                                <div className="absolute z-50 mt-1 max-h-56 w-full overflow-y-auto rounded-md border bg-popover shadow-md">
                                    {disciplineLoading ? (
                                        ...
                                        <p className="px-3 py-2.5 text-xs text-muted-foreground">Không tìm thấy hoặc đã có trong bài</p>
                                    ) : (
