...
import { UpdateBaseInfoProjectMangerForm } from '@/working-manager/project-detail/project-detail-service'
import { updateBaseInfoProjectMangerAction } from '@/working-manager/project-detail/project-detail-hook'
        ...
        <Dialog open={open} onOpenChange={handleClose}>
            ...
            onOpenAutoFocus={(e) => e.preventDefault()}>
                <UpdateBaseProjectForm
                    ...
                    canUpdateBase={!!data?.permissions.canUpdateBase}
                    canUpdateTimeAndLevel={!!data?.permissions.canUpdateTimeAndLevel}
                    onClose={handleClose}
                />
            </DialogContent>
        </Dialog>
...
function UpdateBaseProjectForm({ data, isUpdating, canUpdateBase, canUpdateTimeAndLevel, onClose }: {
        ...
        <>
            ...
            <div className="space-y-4 py-1 overflow-y-auto">
                <div className="space-y-1.5">
                    ...
                    <Input placeholder="Nhập tên đề tài..." value={form.Title} onChange={setField('Title')} disabled={!canUpdateBase} />
                </div>

                <div className="space-y-1.5">
                    <Label className="text-xs">Mô tả <span className="text-red-500">*</span></Label>
                    <Textarea rows={2} placeholder="Mô tả chi tiết..." value={form.Describe} onChange={setField('Describe')} disabled={!canUpdateBase} />
                </div>

                <div className="space-y-1.5">
                    <Label className="text-xs">Cấp độ <span className="text-red-500">*</span></Label>
                    <Select value={form.Level} onValueChange={v => setForm(f => ({ ...f, Level: v }))} disabled={!canUpdateTimeAndLevel}>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                        ...
                        <Input type="date" className="text-sm" value={form.StartDate} onChange={setField('StartDate')} disabled={!canUpdateTimeAndLevel} />
                    </div>
                    <div className="space-y-1.5">
                        <Label className="text-xs">Ngày kết thúc <span className="text-red-500">*</span></Label>
                        <Input type="date" className="text-sm" value={form.EndDate} min={form.StartDate || undefined} onChange={setField('EndDate')} disabled={!canUpdateTimeAndLevel} />
                    </div>
                </div>
            </div>
